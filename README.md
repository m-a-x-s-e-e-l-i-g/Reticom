...
## Latest workspace and network features

- **Map-first Command:** movable Comms, Operators, Tasks and Layers panels, saved layout and Map-only mode. [Workspace guide](docs/command-workspace.md).
- **Multiple teams:** independent team workspaces and a combined overview. [Team management](docs/command-teams.md).
- **Approved membership:** verify device identities before granting team access. [Membership](docs/team-membership.md).
- **Mission snapshots:** late joiners retrieve current markers, drawings, tasks and recent intel; interrupted downloads resume. [Mission sync](docs/mission-sync.md).
- **Experimental team continuity:** explicitly trusted backup hosts replicate state and can take over. Unsynced changes may remain unavailable after host loss. [Continuity and trust boundaries](docs/team-continuity.md).
- **Live phone direction:** encrypted ephemeral heading updates, with device-clock calibration; sharing stops when the app pauses or the screen locks. [Live pointing](docs/live-pointing.md).
- **Public map layers:** disaster areas, FIRMS, ACLED and mapped military areas; flights and vessels with provider-dependent coverage and credentials. Routine loading counters are hidden; source warnings remain. [Providers](docs/intel-providers.md), [traffic](docs/live-traffic.md).
- **Contour lines:** labelled heights in metres, without a separate point-height inspector. [Terrain](docs/elevation.md).
- **Shared navigation and offline routing:** share routes with teammates; region downloads support offline walking/driving where routing data is installed. [Shared routes](docs/shared-navigation.md), [offline navigation](docs/offline-navigation.md).

These guides document current limitations and provider/privacy requirements. Public intel is separate from signed team reports. Additional bundled components retain their licenses in `src/retium/static/vendor/maplibre-contour/` and `src/retium/static/vendor/valhalla/`.
