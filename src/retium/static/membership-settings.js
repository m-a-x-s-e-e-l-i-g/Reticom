export function initMembershipSettings({apiUrl, escapeHtml, toast}) {
  ...
  list.addEventListener("click", async event => {
    ...
    try { render(await request({identity, status, ...(button.dataset.callsign ? {callsign: button.dataset.callsign} : {})})); toast(`Membership ${status}`); }
  ...
  return {update(team, identity) {
    ...
    notice.textContent = status === "revoked" || status === "rejected" ? `Team access ${status}. Contact the team owner. Local work stays on this device.` : "Awaiting team-owner approval. You can use the map locally; team transmissions remain queued.";
